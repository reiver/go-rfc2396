...
func TestBytes(t *testing.T) {
	...
	tests := []struct{
		...
		{
			Value:          []byte("10203040506070809"),
			ExpectedResult: []byte("10203040506070809"),
			ExpectedRest: nil,
			ExpectedOK: true,
		},
		{
...

		{
			...
			ExpectedResult: []byte("192"),
			ExpectedRest:      []byte(".168.0.1"),
			ExpectedOK: true,
		},
	}
	...
	for testNumber, test := range tests {
		...
		actualResult, actualRest, actualOK := decnum.Bytes(test.Value)
